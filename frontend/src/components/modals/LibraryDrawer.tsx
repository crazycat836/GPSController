...
const LibraryDrawer: React.FC<LibraryDrawerProps> = ({ open, onClose }) => {
  ...
  const handleBookmarkClick = useCallback(
    (b: { lat: number; lng: number }) => { sim.handleTeleport(b.lat, b.lng); onClose() },
    [sim, onClose],
  )
  ...
  return (
    <Drawer open={open} onClose={onClose} title="Library" icon={<BookOpen className="w-4 h-4" />} width="w-[min(420px,90vw)]">
      {/* Tab switcher */}
      <div className="flex gap-1 px-4 pt-3 pb-1">
        {(['bookmarks', 'routes'] as const).map((tab) => (
          <button
            key={tab}
            ...
            className={[
              ...
              activeTab === tab
                ...
                : 'text-[var(--color-text-3)] hover:text-[var(--color-text-2)] hover:bg-white/[0.04]',
            ].join(' ')}
          >
            {tab === 'bookmarks'
